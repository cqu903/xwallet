...
- 用于：金额显示、奖励数字、重要数值、高亮信息
- **金色渐变**: 用于奖励卡片、特殊按钮

### 功能色
- **成功**: 绿色 `#4CAF50` - 用于成功状态、正向反馈按钮
- **警告**: 橙色 `#FF9800` - 用于警告提示、限时优惠
- **错误**: 红色 `#F44336` - 用于错误状态、失败提示
- **信息**: 蓝色 `#2196F3` - 用于信息提示、链接

### 背景色
- **主背景**: `#FFFFFF` (亮色) / `#F3F5F7` (次要背景)
- **深色背景**: `#4A148C` (紫色背景) / `#1A1A1A` (深色模式)
- **卡片背景**: `#FFFFFF` (白色卡片) / `#FFFFFF` + 阴影
- **次要背景**: `#F8F9FA` (浅灰背景)
...
✅ **色彩**: 使用紫色主题色 `#7424F5` + 金色强调 `#FFD700`
✅ **字体**: 使用系统默认字体（SF Pro / Roboto），层级清晰
✅ **渐变**: 紫色渐变用于背景，金色渐变用于奖励卡片
✅ **卡片**: 白色卡片 + 轻微阴影，圆角 12px
✅ **动画**: 淡入淡出转场，按钮点击缩放反馈
✅ **圆角**: 统一使用 8/12/16px 圆角
✅ **间距**: 遵循 8/12/16/24px 倍数系统
✅ **图标**: 使用 Material Icons，配合紫色/金色背景
✅ **按钮**: 主按钮 52px 高度，全宽设计
✅ **输入框**: 圆角 12px，聚焦时紫色边框
✅ **激励感**: 金额使用金色/紫色大字体显示
